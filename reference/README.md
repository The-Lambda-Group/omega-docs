...
- [Built-in terms](built-ins.md) -- The complete built-in term inventory. Covers core terms (`=`, `defined`, `return`, `uuid`, `time-now`), arithmetic (`+`, `-`, `*`, `/`, `>`, `<`, `>=`), map operations (`get`, `get-in`, `set`, `merge`, `select-keys`, `zip`), array operations (`get-list`, `length`, `append`, `concat`, `partition`), string operations (`string-split`, `string-concat`, `string-contains`, `string-replace`, `re-find`, `json-stringify`), data transformation patterns (preparing data for `write-table`), the symbols-in-literal-arguments gotcha (why you must bind maps with `=` before passing them to terms), and the HTTP term (`http-request` with request/response map keys, plus deprecated `http-get-request`/`http-post-request`). Keywords: built-in, term, unify, get, set, merge, zip, get-list, json-stringify, json-parse, string-concat, http-request, http-get-request, symbols in literals.
