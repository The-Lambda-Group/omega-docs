...
- [Clauses — Closures / Capture](../reference/clauses.md#closures--capture) — the language-reference home for the `{"capture" [...]}` syntax used to thread a stored functor (e.g. the helper clause from the Fix above) into an implementation clause. Read this when you are declaring captures on the helper for the first time, or when you hit a `clojure.lang.Symbol` runtime error inside a helper that needs to reach an outer-scope binding. Includes the warning about known edge cases with capturing clauses inside other captured clauses — keep capture flat.
