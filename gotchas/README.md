...
- The `run-term` vs `call` rule (per-row freshness, captured-helper dispatch, partition-before-fold) has been consolidated into [`query-omega-oql/docs/explanation/call-vs-run-term-on-captured-helpers.md`](https://github.com/The-Lambda-Group/query-omega-oql/blob/master/docs/explanation/call-vs-run-term-on-captured-helpers.md). That doc covers the per-row UUID / `KEY_UPDATE_CONFLICT` symptom (was `run-term-vs-call-per-row.md`), the captured-helper silent-no-return rule (new), and the unified "when to use which" table.
