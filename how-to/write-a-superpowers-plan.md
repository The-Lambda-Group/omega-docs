...
- **Triage diagnostic step shape (spec and plan authoring).** Every diagnostic step in a triage spec or plan MUST be a minimal OnEvent body with only the clauses needed to answer one question, binding `(= Result {...})` to surface the answer. `(throw ...)` is NOT the inspection mechanic — `throw` surfaces only one solution and aborts the query; `Result` binding surfaces all solutions. The direct-call pattern at the bottom of the impl file (`(= Exec {…})(call OnEvent Exec Result)(return [Result])`) enables fast iteration without `qo push`/`run` dispatch round-trips. **If you find yourself writing `(throw "TRIAGE_SNAPSHOT" ...)` inside a helper predicate in a spec, that step is wrong — rewrite it as a minimal OnEvent with `(= Result {...})`.** This rule is `develop-oql.md` Rule 2, applied to spec authoring.
